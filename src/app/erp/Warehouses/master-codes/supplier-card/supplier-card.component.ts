...
import { DeleteListSupplierCard, SupplierCardDto } from '../../models/supplier-card';
...
export class SupplierCardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  getSuppliers() {
    return new Promise<void>((resolve, reject) => {
      let sub = this.supplierCardService.allSupplierCards(undefined, undefined, undefined, undefined, undefined).subscribe({
        next: (res) => {
  ...
  delete(id: any) {
    this.supplierCardService.deleteSupplierCard(id).subscribe((resonse) => {
      this.getSuppliers();
    });
  }
  edit(id: string) {
    this.router.navigate([
      '/warehouses-master-codes/supplierCard/update-supplierCard',
      id,
    ]);
  }
  ...
  showConfirmDeleteMessage(id) {
    ...
    modalRef.result.then((rs) => {
      if (rs == 'Confirm') {
        ...
        let sub = this.supplierCardService.deleteEntity(input).subscribe(
          (resonse) => {
  ...
  onEdit(id) {
    ...
    if (id != undefined) {
      ...
      this.sharedServices.changeToolbarPath(this.toolbarPathData);
      this.router.navigate(['warehouses-master-codes/supplierCard/update-supplierCard/' + id])
    }
  ...
  onMenuActionSelected(event: ITabulatorActionsSelected) {
    ...
    if (event != null) {
      if (event.actionName == 'Edit') {
        ...
        this.sharedServices.changeToolbarPath(this.toolbarPathData);
        this.router.navigate(['warehouses-master-codes/supplierCard/update-supplierCard/' + event.item.id])
  ...
  onDelete() {
    ...
    let item = new DeleteListSupplierCard();
    ...
    let sub = this.supplierCardService.deleteListEntity(input).subscribe(
      (resonse) => {
