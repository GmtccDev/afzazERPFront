...
import { DeleteListPaymentMethod, PaymentMethodDto } from '../../models/payment-method';
...
export class PaymentMethodComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.spinner.show();
    Promise.all([this.getPaymentMethods()])
      .then(a => {
        this.spinner.hide();
  ...
  getPaymentMethods() {
    return new Promise<void>((resolve, reject) => {
      let sub = this.paymentMethodService.allPaymentMethods(undefined, undefined, undefined, undefined, undefined).subscribe({
        next: (res) => {

          this.toolbarPathData.componentList = this.translate.instant("component-names.payment-method");
          if (res.success) {
            this.paymentMethods = res.response.items
  ...
  delete(id: any) {
    this.paymentMethodService.deletePaymentMethod(id).subscribe((resonse) => {
      this.getPaymentMethods();
    });
  }
  edit(id: string) {
    this.router.navigate([
      '/warehouses-master-codes/paymentMethod/update-paymentMethod',
      id,
    ]);
  }
  ...
  showConfirmDeleteMessage(id) {
    ...
    modalRef.result.then((rs) => {
      ...
      if (rs == 'Confirm') {
        ...
        let sub = this.paymentMethodService.deleteEntity(input).subscribe(
          (resonse) => {
  ...
  onEdit(id) {
    ...
    if (id != undefined) {
      ...
      this.sharedServices.changeToolbarPath(this.toolbarPathData);
      this.router.navigate(['warehouses-master-codes/paymentMethod/update-paymentMethod/' + id])
    }
  ...
  onMenuActionSelected(event: ITabulatorActionsSelected) {
    ...
    if (event != null) {
      if (event.actionName == 'Edit') {
        ...
        this.sharedServices.changeToolbarPath(this.toolbarPathData);
        this.router.navigate(['warehouses-master-codes/paymentMethod/update-paymentMethod/' + event.item.id])
  ...
  onDelete() {
    ...
    let item = new DeleteListPaymentMethod();
    ...
    let sub = this.paymentMethodService.deleteListEntity(input).subscribe(
      (resonse) => {
