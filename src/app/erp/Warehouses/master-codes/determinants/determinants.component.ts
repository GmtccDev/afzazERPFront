...
export class DeterminantsComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	ngOnInit(): void {
	  ...
	  this.spinner.show();
	  Promise.all([this.getDeterminants()])
		.then(a => {
		  this.spinner.hide();
	...
	delete(id: any) {
	  this.determinantsService.deleteDeterminant(id).subscribe((resonse) => {
	...
	edit(id: string) {
	  this.router.navigate([
		'/warehouses-master-codes/determinants/update-determinants',
		id,
	  ]);
	}
	...
	showConfirmDeleteMessage(id) {
	  ...
	  modalRef.result.then((rs) => {
		if (rs == 'Confirm') {
		  this.spinner.show();
		  const input={
			tableName:"DeterminantsMaster",
			id:id,
			idName:"Id"
		  };
		  let sub = this.determinantsService.deleteDeterminant(input.id).subscribe(
			(resonse) => {
	...
	onDelete() {
	  ...
	  let sub = this.determinantsService.deleteDeterminant(ids).subscribe(
		(resonse) => {
